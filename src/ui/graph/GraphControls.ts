...
/**
 * Renders the in-view settings panel using Obsidian's native graph-controls DOM
 * structure and class names (`graph-controls`, `graph-control-section`,
 * `tree-item` / `collapse-icon`, `clickable-icon graph-controls-button`). Reusing
 * those classes means Obsidian's own stylesheet themes the panel, the collapse
 * triangles, and the open/close behavior — including placing the close (X) button
 * exactly where the settings cog was.
 *
 * Pure presentation: reads the current settings, emits change patches, and toggles
 * collapse/open state; it persists nothing itself.
 */
export class GraphControls {
	private rootEl?: HTMLElement;
	// Collapsed state per section title, kept for the view's lifetime. Mirrors
	// Obsidian's defaults (Forces collapsed, the rest expanded).
	private readonly collapsed: Record<string, boolean> = {Forces: true};
	// Panel starts closed: only the cog shows until the user opens it.
	private closed = true;

	constructor(private readonly deps: GraphControlsDeps) {
	}

	/** Builds the `.graph-controls` element inside the given parent. */
	...
	/** Rebuilds the section controls (e.g. after settings change externally). */
	refresh(): void {
		this.renderSections();
	}
	...
	/** A collapsible tree-item section matching Obsidian's graph-control markup. */
