 ...
 * browser. Returns null when neither is exposed. If `--expose-gc` is on we run
 * a collection first so before/after deltas reflect retained (not transient)
 * memory; without it the readings are noisy — read trends, not absolutes.
 */
export function heapUsedMB(): number | null {
	...
	if (typeof performance !== "undefined" && mem && typeof mem.usedJSHeapSize === "number") {
