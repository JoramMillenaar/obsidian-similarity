 ...
 *
 * Answers the single question "how do the JSON, binary, and iodb
 * `IndexStorage` backends perform differently?" without the noise of the full
 * indexing pipeline. Everything above `IndexStorage` (the repo, embed,
 * prepare) is identical across backends, so this benchmark exercises the
 * storage layer *directly* on a synthetic index of N notes.
 *
 ...
 *      parsed-index heap footprint. (json/binary only — iodb has no bare
 *      codec function of its own; its disk cost shows up in section 2.)
 *   2. Real disk round-trip — the actual `rewrite`/`getAll` latency through the
 *      vault {@link DataAdapter} (json, binary) or `IoDbIndexStorage` (iodb),
 *      i.e. the disk cost the in-memory numbers omit.
 *
 * All backends run against *throwaway scratch files* (`index.bench.bin` /
 * `index.bench.json` / `index.bench.iodb.{a,b}.iodb`) that are deleted
 * afterwards, so the live index in `data.json` / `index.bin` is never touched.
 *
...
// ---------------------------------------------------------------------------
// Scratch iodb storage — wraps the real IoDbIndexStorage, but reopens (and
// closes) the database around every call instead of caching it across
// operations. iodb's default durability is debounced (writes land on disk
// some time after the mutating call resolves), which would make `rewrite`
// look free in a naive timing. Forcing a fresh open + close per op mirrors
// what json/binary already do (no in-memory cache survives between calls)
// and makes the measured latency include the actual disk commit.
// ---------------------------------------------------------------------------

class ScratchIoDbIndexStorage implements IndexStorage {
	constructor(
		private readonly adapter: DataAdapter,
	...
	private fresh(): IoDbIndexStorage {
		return new IoDbIndexStorage(this.adapter, this.baseDir, this.dbName);
	}

	async getAll(): Promise<IndexedNote[]> {
		const storage = this.fresh();
		try {
...
/** Sums file sizes across paths — iodb persists each doc across two dual slots. */
async function totalFileSize(adapter: DataAdapter, paths: string[]): Promise<number> {
	let total = 0;
	for (const path of paths) total += await fileSize(adapter, path);
	return total;
}
...
export async function runStorageBenchmark(opts: StorageBenchmarkOptions): Promise<void> {
	...
	const iodbName = "index.bench.iodb";
	const iodbPaths = [
		normalizePath(`${opts.scratchDir}/${iodbName}.a.iodb`),
		normalizePath(`${opts.scratchDir}/${iodbName}.b.iodb`),
	];

	log("════════════════════════════════════════════════════════════════");
	log("  Index storage comparison — JSON vs Binary vs iodb");
	...
	try {
		...
		const backends: { name: string; storage: IndexStorage; diskPaths: string[] }[] = [
			{ name: "json", storage: new ScratchJsonIndexStorage(opts.adapter, jsonPath), diskPaths: [jsonPath] },
			{ name: "binary", storage: new BinaryVaultIndexStorage(opts.adapter, binPath), diskPaths: [binPath] },
			{
				name: "iodb",
				storage: new ScratchIoDbIndexStorage(opts.adapter, opts.scratchDir, iodbName),
				diskPaths: iodbPaths,
			},
		];
		...
		for (const n of sweepSizes) {
			...
			for (let i = 0; i < backends.length; i++) {
				const { name, storage, diskPaths } = backends[i];
				...
				const diskBytes = await totalFileSize(opts.adapter, diskPaths);
		...
		log("   • iodb has no separate codec step (section 1); its rewrite/getAll reopen the database from");
		log("     disk on every call (no cached instance), so its timings already include the full disk cost.");
		log("   • heap deltas are best-effort (no forced GC unless --expose-gc); read the trend.");
		log(`   • scratch files: ${jsonPath}, ${binPath}, ${iodbPaths.join(", ")} (removed on completion).`);
	} finally {
		// Never leave scratch files behind, even if a measurement throws.
		for (const path of [binPath, jsonPath, ...iodbPaths]) {
			try {
