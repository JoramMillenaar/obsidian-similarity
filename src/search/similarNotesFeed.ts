...
import { backendNoticeFor, SimilarNotesNotice, shouldRefreshOnIndexingChange } from "../status/notices";
...
export function makeSimilarNotesFeed(deps: SimilarNotesFeedDeps): SimilarNotesFeed {
	...
	const unsubscribeModelState = backend.subscribeEngineState((next) => {
		...
		if (((modelReady && !wasReady) || next.kind === "error") && noteId !== null) {
			...
			emit({...snapshot, notice: {kind: "warming-up", progress: next.progress}});
		}
	});
